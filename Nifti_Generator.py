...
class SingleNiftiGenerator:
    ...
    def initialize(self, inputX, augOptions=None, normOptions=None, batchTransformFunction=None):
        ...
        x_files = open(inputX)
        x_files_str = x_files.read()
        self.inputFilesX = x_files_str.split(",")
        # trim last empty item
        self.inputFilesX = self.inputFilesX[:len(self.inputFilesX) - 1]
        num_Xfiles = len(self.inputFilesX)
    ...
    def get_batch(self, chunk_size=(32,32,32), batch_size=16):
        ...
        for i in range(batch_size):
            ...
            # Resample images to ensure consistent voxel sizes for all inputs
            # voxel_size = [0.5, 0.5, 1] # what should this be?
            # Ximg = nib.processing.resample_to_output(Ximg, voxel_size)
...
class PairedNiftiGenerator(SingleNiftiGenerator):
    ...
    def initialize(self, inputX, inputY, augOptions=None, normOptions=None, batchTransformFunction=None):
        ...
        y_files = open(inputY)
        y_files_str = y_files.read()
        self.inputFilesY = y_files_str.split(",")
        # trim last empty item
        self.inputFilesY = self.inputFilesY[:len(self.inputFilesY) - 1]
        """
        Lines edited end here
        """
...
# Class to wrap two separate NiftiGens into one output
class UnpairedDualNiftiGenerator():
    @threadsafe_generator
    def generate(X_gen, Y_gen):
        while True:
